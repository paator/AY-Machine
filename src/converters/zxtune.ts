...
export async function convertWithZXTune(request: ConversionRequest): Promise<ConversionResult> {
  ...
  const command = `${TOOLS_DIR}zxtune123 --core-options aym.clockrate="${clock}",aym.layout="${layout}",aym.type="${type}" --mp3 filename="${mp3Path}",bitrate=320 "${inputPath}"`;
