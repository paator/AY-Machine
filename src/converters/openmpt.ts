...
export async function convertWithOpenMPT(request: ConversionRequest): Promise<ConversionResult> {
  ...
  const command = `${TOOLS_DIR}openmpt123 --samplerate 44100 --float --stereo ${flags.openMPTStereoSeparation} --filter ${flags.openMPTUseInterpolation ? 8 : 1} --ctl render.resampler.emulate_amiga=1 --render -- "${inputPath}"`;
